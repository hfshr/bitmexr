...
This vignette gives a brief overview of the main functions within the `bitmexr` package. 
To access the testnet API, use the `tn_` prefix to any of the functions in the package (e.g., `bitmexr::trades()` accesses trade data from the live API whereas `bitmexr::tn_trades()` accesses trade data from the testnet API). This is useful is you want to test out placing/managing orders in a risk free environment.
...
- `bucket_trades()` to get bucketed trade data in 1m, 5m, 1hour or 1day buckets
...
Please see the [Trade data](trade-data.html) vignette for more details on the functions.
...
It is recommended to have a throughough understanding of the bitmex API documentation prior to using the package. Please take some time to read both https://www.bitmex.com/app/apiOverview and https://www.bitmex.com/api/explorer/. 
